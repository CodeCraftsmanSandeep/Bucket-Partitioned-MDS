...
namespace Bucket_Partitioned_MDS
{
        ...
        distance_t& cost) const
    {
        ...
        */
        ...
        const size_t num_nodes = bucket.size();
        ...
        const CVRP& cvrp) const
    {
        ...
        */
        ...
        // Paritioning the problem with exploration + exploitation
        #pragma omp parallel for 
        for(int b = 1; b <= num_buckets; b++) 
        {
            // Finding the vertices in this partition
            const angle_t start_angle = (b-1) * this->alpha;
            const angle_t end_angle = std::min(360.00, b * this->alpha);
            std::vector <node_t> bucket;
            get_bucket(cvrp, start_angle, end_angle, bucket);
            ...
            this->construct_random_mst(cvrp, bucket, mst_adj);
            ...
            for(int _ = 1; _ <= rho; _++)
            {
                ...
                std::vector <std::vector <node_t>> routes;
                distance_t cost = 0.0;
                this->get_routes(cvrp, mst_adj, bucket, routes, cost);
